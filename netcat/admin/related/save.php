...
<head>
    ...
    <script type='text/javascript'>
        try {
            ...
            <? if ($field_id): ?>
                $('#nc_rel_<?= $field_id; ?>_value').val("<?= $object_id; ?>");
                $('#nc_rel_<?= $field_id; ?>_caption').html("<?= $field_caption; ?>");
            <? else: ?>
                $('#cs_<?= $cs_field_name; ?>_value').val("<?= $object_id; ?>");
                ...
                if ($is_adding_mirror_cc) {
                    $('input[name=SubClassName]').val("<?= $caption; ?>");
                    $('input[name=EnglishName]').val("<?= $eng_caption_name; ?>");
                    $('input[name=SrcMirror]').val("<?= $id; ?>");
                    $('input[name=MirrorClassID]').val("<?= $classid; ?>");
                }
            <? endif; ?>
        } catch (e) {

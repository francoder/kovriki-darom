    ...
    function fetch_by_subdivision_id($sub_id) {
        ...
        if (!$this->subdivision_id) {
            ...
            if (nc_module_check_by_keyword('routing')) {
                $this->subdivision_url = nc_routing::get_folder_path($sub_id);
            } else {
...
function nc_customsettings_show_once($ClassID = 0, $TemplateID = 0, $param = '', $Class_Template = 0) {
    ...
    $types = array('string', 'textarea', 'int', 'float', 'checkbox', 'datetime', 'select', 'rel', 'file', 'divider', 'color');
    $subtypes = array();
    $extend_params = array();
    $has_default = array();
...
function nc_customsettings_save_once() {
    ...
    $initial_value = $nc_core->input->fetch_get_post('initial_value');
...
function nc_admin_input($disc, $name, $value = '', $size = 0, $style = '', $simple = '') {
    $attr = array('style' => $style,
        'size' => $size,
        'simple' => $simple,
        'class' => 'input');
    return nc_admin_input_template($name, $value, $attr, "<div class='inf_block'><div>" . $disc . ":</div>", "</div>");
}

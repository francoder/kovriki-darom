...
#######################################################################
    ...
    function  ActionForm($SubClassID, $phase, $type) {
        ...
        if ($type == 1) {   // insert
            ...
            global $EnglishName, $DaysToHold, $AllowTags, $NL2BR, $RecordsPerPage, $MaxRecordsInInfoblock, $MinRecordsInInfoblock, $SortBy, $UseCaptcha, $Class_Template_ID, $isNaked;
            ...
            $SQL = "SELECT `Class_Name`,
                        ...
                        FROM `Class`
                            WHERE `ClassTemplate` = {$Array['Class_ID']}
                              AND `Type` IN ('useful', 'title', 'mobile', 'responsive', 'multi_edit', 'inside_admin')";
...
function ActionForm_for_modal($SubClassID) {
        ...
        $classTemplatesArr = $db->get_results("SELECT `Class_Name`,
                                                   ...
                                                   FROM `Class`
                                                       WHERE `ClassTemplate` = '".$Array['Class_ID']."'
                                                         AND `Type` ".(!$mobile ? "IN ('useful', 'title', 'mobile', 'multi_edit')" : "= 'mobile'"), ARRAY_A);

...
function nc_file_field($field_name, $style = "", $classID = "", $caption = false) {
    ...
    $result .= "<div class='nc-upload'><div class='nc-upload-files'>";
    ...
    if ($message && (($systemTableID && $value) || ($action === 'change' && $old) || ($field_name === 'ncSMO_Image' && $old))) {
        ...
        $file_path = nc_file_path($systemTableID ? $system_tables[$systemTableID] : $classID, $message, $field_id, 'h_');
        ...
        $result .= "<div class='nc-upload-file' id='$block_id' data-type='" . htmlspecialchars($file_type, ENT_QUOTES) . "'>" .
                   ...
                   "<a class='nc-upload-file-name' href='$file_path$file_query'  target='_blank' tabindex='-1'" .
    ...
    # само поле
    $result .= "</div>" .
               "<input class='nc-upload-input' name='f_" . $field_name . "'" . $style_opt . ($style ? " " . $style : "") . " type='file' $accept/>\r\n" .
               "<script>" .
...
function nc_prepare_message_form(   $form, $action, $admin_mode, $user_table_mode, $sys_table_id, $current_cc,
    ...
    if (!CheckUserRights($current_cc['Sub_Class_ID'], "change", 0) || !$admin_mode || (!$inside_admin && !$isNaked)) {
        if ($nc_core->component->can_add_block_markup($current_cc['Class_Template_ID'] ?: $current_cc['Class_ID'])) {
            ...
            $nc_component_css_class = $nc_core->component->get_css_class_name($current_cc['Class_Template_ID'] ?: $current_cc['Class_ID']);

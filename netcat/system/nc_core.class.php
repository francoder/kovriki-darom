...
class nc_Core extends nc_System {
    ...
    public $DOCUMENT_ROOT, $SYSTEM_FOLDER, $INCLUDE_FOLDER, $ROOT_FOLDER, $SUB_FOLDER, $MODULE_FOLDER, $ADMIN_FOLDER,
           ...
           $HTTP_IMAGES_PATH, $HTTP_ROOT_PATH, $HTTP_FILES_PATH, $HTTP_DUMP_PATH, $HTTP_CACHE_PATH, $HTTP_TEMPLATE_PATH,
           $HTTP_TRASH_PATH, $ADMIN_PATH, $ADMIN_TEMPLATE, $ADMIN_TEMPLATE_FOLDER, $NC_JQUERY_PATH, $JQUERY_FOLDER, $MODULE_TEMPLATE_FOLDER,
           $DIRCHMOD, $FILECHMOD;
    ...
    public function output_page_buffer() {
        ...
        if ($this->is_trial && !$this->inside_admin) {
            ...
            if ($trial_message) {
                ...
                $buffer = nc_insert_in_head($buffer, $demo_styles_and_scripts, true);
                $buffer = nc_insert_in_body($buffer, "\n" . $trial_message);
            }
        }

        if (!$this->inside_admin) {
            $buffer = $this->replace_macrofunc($buffer);
            $buffer = $this->page->add_tags_to_output($buffer);
            $buffer = $this->page->insert_partials_script($buffer);
        }

        foreach ($this->output_processors as $fn) {
            $buffer = call_user_func($fn, $buffer);
        }

        $buffer = $this->security->xss_filter->filter($buffer);

        if (!$this->admin_mode) {
            // Отправка заголовка 304, если страница не изменилась с момента последнего запроса
            $this->page->send_and_check_cache_validator_headers($buffer);
        }

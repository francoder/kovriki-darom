...
class nc_tpl {
    ...
    public function update_relative_path_last_fragment($new_fragment) {
        if (!$this->type || !preg_match('/^\w+$/', $new_fragment)) {
            return false;
        }
        ...
        $new_relative_path = preg_replace('!/\w+/$!', "/$new_fragment/", $old_relative_path);
        $new_absolute_path = preg_replace('!/\w+/$!', "/$new_fragment/", $old_absolute_path);

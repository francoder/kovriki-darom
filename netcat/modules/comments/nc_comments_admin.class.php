...
class nc_comments_admin {
    ...
    public function converterSave() {
        ...
        $catalogue = intval($this->POST['ConverterCatalogue']);
        $subdivision = intval($this->POST['ConverterSubdivision']);
        $subclass = intval($this->POST['ConverterSubClass']);
    ...
    public function essence_select_form($param, $nochilds, $nouser) {
        ...
        $ret.= nc_select_options($this->get_catalogues(), $param['site'], 0, 0, 1);
        ...
        if ($param['site']) {
            ...
            $ret.= !empty($subs) ? nc_select_options($subs, $param['subid'], 0, 0, 1) : "<option value=\"0\">".NETCAT_MODULE_COMMENTS_ADMIN_NONE_SUB."</option>\n";
        }

        $ret.= "</select></div></td></tr>\n";

        $ret.= "<tr ".($nochilds ? "style='display:none'" : "")."><td></td>
            <td><input type='checkbox' name='wholesub' id='wholesub' value='1' ".($param['wholesub'] ? "checked" : "")." ".($param['subid'] ? "" : "disabled")." onclick='nc_filter.check_wholesub();'/>
        ...
        if ($param['subid']) {
            ...
            $ret.= !empty($ccs) ? nc_select_options($ccs, $param['cc'], 0, 0, 1) : "<option value=\"0\">".NETCAT_MODULE_COMMENTS_ADMIN_NONE_CC."</option>\n";
        }

        $ret.= "</select></div></td></tr>\n
            <tr><td>" . NETCAT_MODULE_COMMENTS_ADMIN_MESSAGE . "</td>
            <td><div>
                <input type='text' name='message' id='input_message'  value='".$param['message']."' ".($param['cc'] || $param['message'] ? "" : "disabled")." style='width: 30%'/>
                <button id='select_message' onclick='nc_filter.select_message(); return false;' ".($param['cc'] || $param['message'] ? "" : "disabled").">". NETCAT_MODULE_COMMENTS_ADMIN_FROM_LIST ."</button>
            </div></td></tr>\n";

        $ret.= "<tr ".($nouser ? "style='display:none'" : "").">
                <td>" . NETCAT_MODULE_COMMENTS_ADMIN_USER. "</td>
                <td><div>
                <input type='text' name='user_name' id='input_user'  value='".$param['user_name']."' style='width: 65%'/>
        ...
        $ret.=  "
            <script type='text/javascript' src='" . nc_add_revision_to_url('admin.js') . "'></script>
    ...
    public function search_comments_form($qty) {
  	...
  	<form method='post' action='admin.php' id='CommentSearchForm'>";
        ...
        $html .= $this->essence_select_form($this->POST, 0, 0);
    ...
    public function listing($cc_env, $range) {
        ...
        $const_url = $cc_env['LocalQuery'] . ($phase ? "&phase=".(int) $phase : "") . ($order_by ? "&order_by=" . (int)$order_by : "&order_by=0") . ($sort_by == 1 ? "&sort_by=1" : $sort_by == 2 ? "&sort_by=2" : "");
        if ($const_url == '?')
            $const_url = '';
    ...
    public function comments_list() {
        $nc_core = nc_Core::get_object();
        extract((array) $this->POST);

        if ($user_name) {
            $user_id = $this->db->get_var("SELECT `User_ID` as user FROM User WHERE `".$nc_core->AUTHORIZE_BY."` = '". $this->db->escape($user_name)."'");
        ...
        elseif ($cc > 0) {
            $where .= "AND c.`Sub_Class_ID` = " . (int)$cc . " ";
        }
        elseif ($subid > 0) {
            if ($wholesub) {
        ...
        elseif ($site > 0) {
            $where .= "AND sc.`Catalogue_ID` = " . (int)$site . " ";
        }
        ...
        $get_param = array('message', 'cc', 'sub', 'catalogue', 'wholesub', 'checked', 'user_id');
...
" . (!$comment['Checked'] ? "style='color:#cccccc;'>" . nc_bbcode($comment['Comment']) :
    ...
    public function subscribe_settings() {
        ...
        // get settings
        $phase = $nc_core->input->fetch_get_post('phase');
        $reload = $phase == 31 ? 1 : 0;
        ...
        // auto subscribe
        echo nc_admin_checkbox(NETCAT_MODULE_COMMENTS_ADMIN_SUBSCRIBE_SETTINGS_AUTO, 'Subscribe_Auto', $settings['Subscribe_Auto']);
        ...
        echo nc_admin_input(NETCAT_MODULE_COMMENTS_ADMIN_SUBSCRIBE_SETTINGS_ADMIN_EMAIL, 'Mail_Email_Admin', $settings['Mail_Email_Admin'] ? $settings['Mail_Email_Admin'] : $SPAM_FROM, 0, 'width:100%;');
    ...
    public function subscribe_form() {
        ...
        $result .= $this->essence_select_form($this->POST, 1, 0);
    ...
    public function subscribe_form_save() {
        ...
        $where = '';
        if ($message) {
            $where = "`Message_ID` = ".(int)$message." AND `Sub_Class_ID` = ".(int)$cc." ";
        }
        else if ($cc) {
            $where = "`Message_ID` = 0 AND `Sub_Class_ID` = ".(int)$cc." ";
        }
        else if ($subid) {
            $where = "`Message_ID` = 0 AND `Sub_Class_ID` = 0 AND `Subdivision_ID` = ".(int)$subid." ";
        }
        else {
            $where = "`Message_ID` = 0 AND `Sub_Class_ID` = 0 AND `Subdivision_ID` = 0 AND `Catalogue_ID` = ".(int)$site." ";
        }
        $subscribed = $this->db->get_var("SELECT `ID` 
        ...
        $this->db->query("INSERT INTO `Comments_Subscribe`
            (`User_ID`, `Message_ID`, `Sub_Class_ID`, `Subdivision_ID`, `Catalogue_ID`, Comment_ID)
               VALUES
            (".(int)$user_id.", ".(int)$message.", ".(int)$cc.", ".(int)$subid.", ".(int)$site.", 0)");
    ...
    public function subscribes_delete_confirm() {
        $nc_core = nc_Core::get_object();
        extract((array) $this->POST);
        $subscribe_arr = (array)$subscribe_arr;
   	...
   	function sumbit_form ( phase ) {\n
        ...
        $data = $this->db->get_results("SELECT c.*, u.`Login`, u.`User_ID`
    ...
    public function subscribes_delete() {
        $subscribe_arr = (array)$this->POST['subscribe_arr'];

        if (empty($subscribe_arr) && $this->POST['subscribe_id']) {
            $subscribe_arr[] = $this->POST['subscribe_id'];
        }
    ...
    public function subscribes_list() {
   	...
   	function sumbit_form ( phase ) {\n
  			...
  			<tr>
  				<td>
  					<table class='admin_table' width=100%>
  						<tr>
  							<td><a href='admin.php?phase=1&sort_by=1&order_by=" . (!$order_by ? !$order_by : 0) . "&qty=" . $qty . $url . "'>ID</a>" . (($order_by && $sort_by == 1) ? "&nbsp;&nbsp;&#x2191;" : ($sort_by == 1 ? "&nbsp;&nbsp;&#x2193;" : "")) . "</td>
  							<td><a href='admin.php?phase=1&sort_by=2&order_by=" . (!$order_by ? !$order_by : 0) . "&qty=" . $qty . $url . "'>" . NETCAT_MODULE_COMMENTS_ADMIN_SUBSCRIBERS_USER . "</a>" . (($order_by && $sort_by == 2) ? "&nbsp;&nbsp;&#x2191;" : ($sort_by == 2 ? "&nbsp;&nbsp;&#x2193;" : "")) . "</td>
  							<td width=50%>" . NETCAT_MODULE_COMMENTS_ADMIN_ESSENCE . "</td>
  							<td align=right width=20%>" . CONTROL_USER_ACTIONS . "</td>
        ...
        foreach ($data as $subscribe) {
            echo "<tr>
    	<td>" . $subscribe['ID'] . "</td>
    	<td><a href='" . $nc_core->ADMIN_PATH . "user/index.php?phase=4&UserID=" . $subscribe['User_ID'] . "' >" . $subscribe['Login'] . "</a></td>
    	<td>";
        if ($subscribe['Message_ID']) {
        ...
        if ($qty || $curPos) {
            ...
            if ($pagination) {
                ...
                // last link
                if ($curPos != $last_pos && (($range + 1) * $maxRows) < $totRows) {
                    echo "&nbsp;&nbsp;<a href='" . $env['LocalQuery'] . "&curPos=" . $last_pos . "". ($phase ? "&phase=".(int) $phase : "") ."'>&raquo;&raquo;</a>";
                }
    ...
    public function settings() {
        ...
        echo nc_admin_checkbox(NETCAT_MODULE_COMMENTS_ADMIN_SETTINGS_ADD_BLOCK_FIRST, 'AddBlockFirst', $settings['AddBlockFirst']);
        ...
        echo nc_admin_checkbox(NETCAT_MODULE_COMMENTS_ADMIN_SETTINGS_AUTH_BUTTONS, 'Auth_Buttons', $settings['Auth_Buttons']);
    ...
    public function template() {
        ...
        $Template = $this->template ? $this->template : $this->POST['Template'];

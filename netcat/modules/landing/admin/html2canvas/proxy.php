...
// нужен относительный путь для временной папки
$doc_root_length = strlen($nc_core->DOCUMENT_ROOT);
$current_folder = str_replace(DIRECTORY_SEPARATOR, '/', substr(__DIR__, $doc_root_length));
$root = preg_replace('!/[^/]+!', '/..', $current_folder);
$root = substr($root, 1);

$tmp_folder = $nc_core->TMP_FOLDER . 'html2canvas';
$tmp_folder = substr($tmp_folder, $doc_root_length);
$tmp_folder = $root . $tmp_folder;

define('PATH', $tmp_folder); //relative folder where the images are saved
...
//setup
define('JSLOG', ''); //Configure alternative function log, eg. console.log, alert, custom_function

...
class phpMorphy_Morphier_Empty implements phpMorphy_Morphier_Interface {
    ...
    function castFormByGramInfo($word, $partOfSpeech, $grammems, $returnWords = false, $callback = null) {
        return false;
    }
...
class phpMorphy_Morphier_Helper {
    ...
    function __construct(
    phpMorphy_GramInfo_Interace $graminfo, phpMorphy_GramTab_Interface $gramtab, phpMorphy_AncodesResolver_Interface $ancodesResolver, $resolvePartOfSpeech
    ) {
...
class phpMorphy_Morphier_Finder_Predict_Databse extends phpMorphy_Morphier_Finder_Common {
    ...
    function __construct(
    phpMorphy_Fsa_Interface $fsa, phpMorphy_AnnotDecoder_Interface $annotDecoder, $encoding, phpMorphy_GramInfo_Interace $graminfo, $minPostfixMatch = 2, $collectLimit = 32
    ) {
        parent::__construct($fsa, $annotDecoder);

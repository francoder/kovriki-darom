...
<?php if (!empty($result['data']) && is_array($result['data']) && count($result['data']) > 0): ?>
    <div class="nc-panel-content nc-bg-lighten">
        <div class="nc-padding-20">
            <div class='nc-chart' id='nc_geo_chart'></div>
        </div>
    </div>
    <div class="nc-panel-content nc-bg-lighten">
        ...
        <div class="nc--left nc-padding-20">
            <table class='nc-table nc--bordered nc--striped'>
                <tr>
                    <th></th>
                    <th><?php echo NETCAT_MODULE_SERVICES_METRIKA_STAT_GEO ?></th>
...
<script type="text/javascript">
    ...
    (function() {
        ...
        if (nc.key_exists('total', stat)) {
            nc.chart(nc('#nc_geo_chart'), stat.data, {series: {pie: { show: true} }, legend: {show: true}, grid: {hoverable: true, clickable: false}});
        } else {
            nc('#nc_geo_chart').html('<div class="nc-padding-20"><?= NETCAT_MODULE_SERVICES_DATA_NOT_AVAILABLE ?></div>');
        }
    })();
</script>
    ...
    function CartPut($array, $mode = "", $custom_params = array()) {
        ...
        foreach ((array) $array as $type_id => $arr) {
            ...
            foreach ($fields as $k => $v) {
                if ($v['name'] == 'StockUnits' && $v['type'] == NC_FIELDTYPE_FLOAT) {
...
//         include_once('Mail.php');
            ...
            $host = $nc_core->catalogue->get_current('Domain') ?: $_SERVER['HTTP_HOST'];
            $header = sprintf(NETCAT_MODULE_NETSHOP_EMAIL_TO_MANAGER_HEADER, decode_host($host));
            ...
            foreach ((array) $this->SendMails as $to => $row) {
                ...
                if ($order_tpl) {
                    $body .= "\n" . $nc_core->catalogue->get_url_by_id($GLOBALS['catalogue']) . $SUB_FOLDER . $HTTP_ROOT_PATH . "message.php?catalogue=$GLOBALS[catalogue]&sub=$order_tpl[Subdivision_ID]&cc=$order_tpl[Sub_Class_ID]&message=$this->OrderID&curPos=0\n";
                }

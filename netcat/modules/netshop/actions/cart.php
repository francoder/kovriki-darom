...
foreach ($cart as $component_id => $items) {
    foreach ((array)$items as $item_id => $qty) {
        ...
        $additional_params = $cart_params;
        if (isset($item_params[$component_id][$item_id])) {
            $additional_params = $item_params[$component_id][$item_id];
            foreach ($additional_params as $key => $param) {
                $paramComponentId = (int)getComponentIdFromCompItemStr($param);
                $paramItemId = (int)getItemIdFromCompItemStr($param);
                 $additional_params['count'][$paramComponentId][$paramItemId] = $cart[$paramComponentId][$paramItemId];
            }
        }
...
if ($input->fetch_post('json')) {
    ...
    $item_properties = [
        'Class_ID', 'Message_ID', 'Name', 'VariantName', 'Vendor',
    ...
    if ($quantity_notifications) {
        $data['QuantityNotifications'] = [];
        foreach ($quantity_notifications->get_all() as $notification) {
            $item = $notification['item'];
            $data['QuantityNotifications'][$item['_ItemKey']] = [

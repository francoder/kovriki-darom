 ...
 * @property nc_netshop_location $location
 */
class nc_netshop {

    // События для интеграции с модулем статистики (см. также метод register_events)
    // — изменение количества товара в корзине: (nc_netshop_item $item, $qty_change)
    const EVENT_AFTER_CART_CHANGED = 'netshopCartChanged';
    // — оформление заказа через метод place_order: (nc_netshop_order $order, array $coupon_codes)
    const EVENT_AFTER_ORDER_CREATED = 'netshopOrderCreated';
    ...
    /**
     * Регистрирует события для статистики e-commerce в nc_event.
     */
    static public function register_events() {
        $nc_core = nc_core::get_object();
        // события не будут регистрироваться логгером
        $nc_core->event->register_event(self::EVENT_AFTER_CART_CHANGED, '', true);
        $nc_core->event->register_event(self::EVENT_AFTER_ORDER_CREATED, '', true);
    }
     ...
     *
     */
    public function set_order_in_condition_context(nc_netshop_order $order = null) {
    ...
    public function place_order(nc_netshop_order $order, $reg_data = array()) {
        ...
        $order->mark_as_being_created(true);
        ...
        // информация о купонах для внешней аналитике e-commerce
        $coupons = $this->promotion->get_registered_coupons();
        if ($coupons) {
            $coupon_codes = $coupons->each('get', 'code');
        } else {
            $coupon_codes = array();
        }
        ...
        $order->mark_as_being_created(false);

        // записать накопленные изменения в заказ:
        $order->save();

        // событие (используется для аналитики e-commerce Google/Яндекса)
        nc_core::get_object()->event->execute(self::EVENT_AFTER_ORDER_CREATED, $order, $coupon_codes);
    }
    ...
    /**
     * onPayRejected event handler
     *
     * @param nc_payment_system $payment_system
     * @param nc_payment_invoice $invoice
     */
    public function on_payment_rejected_event_handler(nc_payment_system $payment_system, nc_payment_invoice $invoice = null) {
            if ($invoice && $invoice->get('order_source') == 'netshop') {
            $order_id = $invoice->get('order_id');

            $order = $this->load_order($order_id);
            if ($order) {
                $order->set('Status', $this->get_setting('RejectedOrderStatusID'));
                $this->mailer->process_order_status_change($order);
                $order->save();
            }
        }

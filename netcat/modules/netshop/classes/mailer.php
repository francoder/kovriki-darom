...
class nc_netshop_mailer {
    ...
    public function get_sender_address() {
        ...
        if ($this->netshop->get_setting('MailFrom')) {
            return $this->netshop->get_setting('MailFrom');
        }

        if ($nc_core->get_settings('SpamFromEmail')) {
            return $nc_core->get_settings('SpamFromEmail');
        }

        $catalogue_domain = $nc_core->catalogue->get_current('Domain');
        $catalogue_default_email = 'no-reply@' . $catalogue_domain;

        if ($catalogue_domain && filter_var($catalogue_default_email, FILTER_VALIDATE_EMAIL)) {
            return $catalogue_default_email;
        }
    ...
    public function get_sender_name() {
        ...
        if ($this->netshop->get_setting('ShopName')) {
            return $this->netshop->get_setting('ShopName');
        }

        if ($nc_core->get_settings('SpamFromName')) {
            return $nc_core->get_settings('SpamFromName');
        }
        ...
        if (!$fields) {
            $results = nc_core('db')->get_results("SELECT `Field_Name`, `Description`
                                                    ...
                                                    WHERE `System_Table_ID` = '3'
                                                      AND `TypeOfData_ID` = " . NC_FIELDTYPE_STRING . "
                                                      AND `Format` = 'email'

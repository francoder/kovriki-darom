...
class nc_netshop_external_order {
    ...
    public function __construct($order_data = null) {
        ...
        if (!empty($order_data['delivery_variant_id'])) {
            $this->order->set_delivery_variant_id($order_data['delivery_variant_id']);
        }
        if (!empty($order_data['delivery_point_id'])) {
            $this->order->set_delivery_point_id($order_data['delivery_point_id']);
        }
        $this->order->save();

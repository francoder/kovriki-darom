...
class nc_netshop_item implements ArrayAccess {
    ...
    static public function by_id($component_id, $item_id) {
        return new static(array('Class_ID' => $component_id, 'Message_ID' => $item_id));
    }
    ...
    protected function select_children($only_checked) {
        ...
        if (!$this->has_parent()) {
            ...
            $rows = (array)$nc_core->db->get_results($query, ARRAY_A);
            foreach ($rows as $row) {
                $item = new nc_netshop_item($row);
                $item->mark_as_loaded();
                $item->offsetSet('_Parent', $this);
                $result->add($item);
                $nc_core->page->update_last_modified_if_timestamp_is_newer(strtotime($item->get_defined_property('Created')));
                $nc_core->page->update_last_modified_if_timestamp_is_newer(strtotime($item->get_defined_property('LastUpdated')));
            }
        }
        return $result;
    }
    ...
    protected function get_discount_info() {
        ...
        foreach ($discounts as $discount) {
            ...
            $max_discount = $netshop->round_price(round($price - $min_price, 2));
            $is_min_price_reached = $discount_sum >= $max_discount;
            if ($is_min_price_reached) {
            ...
            $discount_info[] = array(
                ...
                'sum' => $discount_sum,
                'price_minimum' => (int)$is_min_price_reached,
            );
    ...
    protected function get_select_query_fields() {
        ...
        if (!$cache[$component_id]) {
            ...
            $cache[$component_id] =
                "SELECT " . $component->get_fields_query() . ", $component_id AS `Class_ID`\n" .
    ...
    protected function load() {
        ...
        if ($id) {
            ...
            if ($data) {
                ...
                $page->update_last_modified_if_timestamp_is_newer(strtotime($this->get_defined_property('Created')));
                $page->update_last_modified_if_timestamp_is_newer(strtotime($this->get_defined_property('LastUpdated')));
            }
            else {
    ...
    public function price_depends_on_user_data() {
        ...
        foreach ($user_dependent_conditions as $condition_type) {
            ...
            if ($discounts->any('has_condition_of_type', true, '==', array($condition_type))) {
                return true;
            }
        }
    ...
    /**
     * Добавляет скидку. Если скидка с таким названием уже существует, сумма скидки
     * добавляется к этой скидке.
     * Данный метод предназначен только для внутреннего использования совместно
     * с методом nc_netshop_order::get_get_items_with_distributed_cart_discount().
     * Добавленные с помощью этого метода скидки не будут сохранены при сохранении
     * заказа в БД.
     *
     * @internal
     * @param $name
     * @param $added_discount_sum
     */
    public function add_custom_discount($name, $added_discount_sum) {
        $discounts = $this['Discounts'];
        $existing_discount_sum = $this['ItemDiscount'];
        $this->reset_calculated_prices();
        unset($this->item_data['ItemPrice']); // для товаров, загруженных из заказа

        $discount_exists = false;
        foreach ($discounts as $i => $discount) {
            if ($discount['id'] === 0 && $discount['name'] === $name) {
                $discounts[$i]['sum'] += $added_discount_sum;
        ...
        if (!$discount_exists) {
            $discounts[] = array(
                ...
                'name' => $name,
                'description' => '',
                'sum' => $added_discount_sum,
                'price_minimum' => 0,
            );
        }

        $this->additional_data['Discounts'] = $discounts;
        $this->additional_data['ItemDiscount'] = round($existing_discount_sum + $added_discount_sum, 2);
    }
     ...
     * Currency в названии
     */
    protected function reset_calculated_prices() {
    ...
    public function offsetUnset($offset) {
        unset($this->additional_data[$offset]);
    }

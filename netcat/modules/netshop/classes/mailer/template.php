...
class nc_netshop_mailer_template extends nc_record {
    ...
    protected function get_field_type_and_value($item, $item_field, $item_type, $site_id) {
        ...
        elseif ($item_type == 'order') {
            $is_html_allowed =
                // а) special fields with "F" suffix (formatted prices): no html entities encoding
                substr($item_field, -1) === 'F' ||
                // б) delivery point description can contain HTML (e.g. a link to the delivery point page)
                $item_field === 'DeliveryPointDescription';

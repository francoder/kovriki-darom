...
class nc_netshop_cart {
    ...
    public function add_item($component_id, $item_id, $qty = 1, $replace_existing = true, array $additional_params = null) {
        ...
        // Изменение количества (для событий статистики e-commerce)
        $qty_delta = $qty;

        // Товар уже есть в корзине?
        if (isset($this->raw_contents[$component_id][$item_id])) {
            $row = &$this->raw_contents[$component_id][$item_id];
            if ($replace_existing) { // заменяем количество и параметры
                $qty_delta = $qty - $row['Qty'];
            } else { // добавляем к количеству в корзине, объединяем параметры
                $qty += $row['Qty'];
...
//        die;
        ...
        $this->on_qty_change($item, $qty_delta);
    ...
    /**
     * Вызов события для сопряжения с внешними скриптами аналитики (модуль stats)
     * @param nc_netshop_item $item
     * @param int|float|string $qty_change
     */
    protected function on_qty_change(nc_netshop_item $item, $qty_change) {
        if ($qty_change) {
            nc_core::get_object()->event->execute(nc_netshop::EVENT_AFTER_CART_CHANGED, $item, $qty_change);
        }
    }

...
class nc_netshop_order_admin_controller extends nc_netshop_admin_controller {
    ...
    protected function action_get_delivery_method_list() {
        ...
        $order_data = $this->input->fetch_post();
        // При создании заказа через from_post_order будет произведен запрос данных
        // о способе доставки для внешних служб доставки с вариантами. Этот запрос
        // нам не нужен, так как мы получаем список всех возможных способов доставки
        // и вообще изменяем его. Избавляемся от данных о способе доставки в заказе:
        unset($order_data['delivery_variant_id'], $order_data['delivery_point_id']);

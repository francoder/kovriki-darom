...
class nc_netshop_order extends nc_record {
    ...
    protected $is_being_created = false;
...
//            'User_ID' => nc_core::get_object()->user->get_current('User_ID'),
    ...
    /**
     * Обновляет значения TotalPrice и TotalGoods.
     * @return $this
     */
    protected function update_totals() {
    ...
    public function get_cart_discounts($type = null) {
        if ($this->cart_discounts === null) {
            if ($this->is_being_created || !$this->get_id()) {
                $promo = $this->get_netshop()->promotion;
                $this->cart_discounts = array_merge(
                    $promo->get_cart_discount_info(),
                    $promo->get_delivery_discount_info($this)
                );
            } else {
    ...
    /**
     * Возвращает коллекцию с товарами в заказе, где на цену товаров распределена
     * скидка на корзину (для чеков, доставки и т. п.)
     *
     * @param bool $use_currency_rounding_setting округлять в соответствии с параметрами основной валюты
     * @return nc_netshop_item_collection
     */
    public function get_items_with_distributed_cart_discount($use_currency_rounding_setting = false) {
        $discount_remainder = $total_discount = $this->get_cart_discount_sum();

        $netshop = $this->get_netshop();
        $order_items = $this->get_items();

        $rounding_precision = $use_currency_rounding_setting
            ? $netshop->get_setting('CurrencyDetails', $netshop->get_setting('DefaultCurrencyID'), 'Decimals')
            : 2;
        $price_unit = pow(10, -1 * $rounding_precision); // шаг изменения цены

        if (!$total_discount || $total_discount <= $price_unit) {
            return $order_items;
        }

        $totals = $order_items->sum('TotalPrice');
        $result = clone $order_items;

        /** @var nc_netshop_item $item */
        foreach ($result as $item) {
            $original_total_price = $item['TotalPrice'];
            $share = $original_total_price / $totals;
            $discount = round($total_discount * $share / $item['Qty'], $rounding_precision);
            // Скидка должна быть не меньше, чем шаг изменения цены:
            $discount = max($discount, $price_unit);
            // и не больше стоимости товара (но не 0) или остатка скидки:
            $discount = min($discount, $item['ItemPrice'] - $price_unit, $discount_remainder);

            $item->add_custom_discount(NETCAT_MODULE_NETSHOP_CART_DISCOUNT, $discount);

            $discount_remainder -= $original_total_price - $item['TotalPrice'];
            $discount_remainder = round($discount_remainder, $rounding_precision);
        }

        // Если остался нераспределённый остаток (или распределённая скидка больше скидки на корзину),
        // ищем товар, цену которого можно скорректировать
        if (abs($discount_remainder) >= $price_unit) {
            foreach ($result as $item) {
                $discount_per_item = round($discount_remainder / $item['Qty'], $rounding_precision);
                if ($discount_per_item >= $price_unit && $item['ItemPrice'] > $price_unit) {
                    $item->add_custom_discount(NETCAT_MODULE_NETSHOP_CART_DISCOUNT, $discount_per_item);
                    break;
                }
            }
        }
    ...
    protected function create_stock_fields() {
        ...
        $db->query("INSERT INTO `Field`
                    SET `Class_ID` = $order_component_id,
                        ...
                        `DefaultState` = '0',
                        `TypeOfData_ID` = " . NC_FIELDTYPE_BOOLEAN . ",
                        `Priority` = $priority,
                        `Checked` = 1,
    ...
    /**
     * Отмечает, что в настоящий момент заказ оформляется
     *
     * @param bool $is_being_created
     */
    public function mark_as_being_created($is_being_created = true) {
        $this->is_being_created = $is_being_created;
    }

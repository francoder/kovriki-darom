...
class nc_netshop_promotion {
    ...
    /**
     * Возвращает массив с информацией о скидках на корзину (для текущей корзины)
     * (в виде, который используется в nc_netshop_order)
     * @return array
     */
    ...
    protected function save_order_cart_discounts(nc_netshop_order $order) {
        foreach ($this->get_cart_discount_info() as $discount_info) {
            $this->save_discount_info($order->get_id(), 0, 0, $discount_info);
        }
    }

    /**
     * Возвращает массив с информацией о действующих скидках на доставку для заказа
     * (в виде, который используется в nc_netshop_order)
     * @param nc_netshop_order $order
     * @return array
     */
    public function get_delivery_discount_info(nc_netshop_order $order) {
        if (!$this->are_delivery_discounts_enabled) {
            return array();
        }
    ...
    protected function save_order_delivery_discounts(nc_netshop_order $order) {
        foreach ($this->get_delivery_discount_info($order) as $discount_info) {
            $this->save_discount_info($order->get_id(), 0, 0, $discount_info);
        }
    }

...
class nc_netshop_filter {
     ...
     * @param int $cc id инфоблока
     * @return array  $fields
     */
    public function init_fields($fields = array(), $cc = null) {
        static $filter_fields;

        if ($cc) {
            $this->infoblock_id = $cc;
        }
        ...
        if (!$this->component_id) {
            try {
                if ($this->infoblock_id) {
                    $this->component_id = nc_Core::get_object()->sub_class->get_by_id($this->infoblock_id, 'Class_ID');
                } else {
                    $this->component_id = nc_Core::get_object()->sub_class->get_current('Class_ID');
                }
            } catch (Exception $e) {
    ...
    public function get_infoblock_id() {
        if (!$this->infoblock_id) {
            if ($this->options('ignore_cc')) {
                $this->infoblock_id = null;
            }
            else {
                try {
                    $this->infoblock_id = nc_Core::get_object()->sub_class->get_current('SrcMirror') ?:
                        nc_Core::get_object()->sub_class->get_current('Sub_Class_ID');
                } catch (Exception $e) {
                    $this->infoblock_id = null;
                }
            }
